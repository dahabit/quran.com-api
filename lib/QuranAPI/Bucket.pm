        ...
        my $result = $self->cache( join( '.', 'audio', $vars{audio}, @keys ) => sub {
            ...
            my @result = $self->db->query( qq|
                select a.ayah_key
                  ...
                  from audio.file f
                  join quran.ayah a using ( ayah_key )
                  left join ( select t.recitation_id
                     , f.ayah_key
                 ...
                 where f.recitation_id = ?
                   and a.ayah_key in ( |.( join ', ', map { '?' } @keys ).qq| )
                 group by a.ayah_key, ogg.url, ogg.duration, ogg.mime_type, mp3.url, mp3.duration, mp3.mime_type
                 order by a.surah_id, a.ayah_num
            |, $vars{audio}, @keys )->hashes;
            for my $result ( @result ) {
                my $ayah_key = delete $result->{ayah_key};
                my %mash;
                   $mash{ogg} = [ qw/url duration mime_type/ ];
                   $mash{mp3} = [ qw/url duration mime_type/ ];
                for my $pfix ( keys %mash ) {
                    my ( @grep, %hash );
                    @grep = grep { $_ =~ /^${pfix}_/ } keys %{ $result };
                    for my $orig ( @grep ) {
                        my $attr = $orig;
                           $attr =~ s/^${pfix}_//;
                        $hash{ $attr } = delete $result->{ $orig };
